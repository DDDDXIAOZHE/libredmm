...
RSpec.feature 'List movies with baidu pan resource filter', type: :feature do
  before :each do
    @user = create :user
  ...
  context 'any' do
    scenario 'when signed out' do
      visit movies_url(baidu_pan_resource: 'any')
      expect(page).to have_selector('.movie', count: Movie.count)
    end
  end
  ...
  context 'unknown' do
    scenario 'when signed out' do
      visit movies_url(baidu_pan_resource: 'unknown')
      expect(page).to have_selector('.movie', count: Movie.count)
    end
  end
  ...
  context 'with' do
    scenario 'when signed in' do
      visit movies_url(baidu_pan_resource: 'with', as: @user)
    ...
    scenario 'when signed out' do
      visit movies_url(baidu_pan_resource: 'with')
      expect(page).not_to have_selector('.movie')
    end
  ...
  context 'without' do
    scenario 'when signed in' do
      visit movies_url(baidu_pan_resource: 'without', as: @user)
      expect(page).not_to have_selector('.movie')
    end
    ...
    scenario 'when signed out' do
      visit movies_url(baidu_pan_resource: 'without')
      expect(page).not_to have_selector('.movie')
    end
