...
RSpec.feature 'List movies with resource filter', type: :feature do
  ...
  context 'any' do
    ...
    scenario 'when signed in as admin' do
      ...
      expect(page).to have_selector('.card-body', count: Movie.with_resources.count)
    end
    ...
    scenario 'and chained after vote filter' do
      ...
      expect(page).to have_selector('.card-body', count: 1)
    end
  end
  ...
  context 'none' do
    ...
    scenario 'when signed in as admin' do
      ...
      expect(page).to have_selector('.card-body', count: Movie.without_resources.count)
    end
